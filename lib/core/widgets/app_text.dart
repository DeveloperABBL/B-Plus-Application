import 'package:brownyplus/core/utils/app_extensions.dart';
...
class AppText extends StatelessWidget {
  ...
  AppText.tiny(
    ...
    this.height,
    super.key,
  }) : style = style ?? AppTextStyles.labelSmall;

  static final RegExp _regExp = RegExp(r'(?<=\d)(?=\D)|(?=\d)(?<=\D)');
  static final RegExp _number = RegExp(r'[\d$฿]');

  @override
  Widget build(BuildContext context) {
    TextStyle mStyle = (style ?? context.textTheme.labelLarge!).copyWith(
      color: color,
      fontSize: fontSize,
      fontWeight: fontWeight,
      height: height,
    );
    final List<String> split = text.split(_regExp);
    return RichText(
      text: TextSpan(
        children: split
            .map(
              (e) => _number.hasMatch(e)
                  ? TextSpan(
                      text: e,
